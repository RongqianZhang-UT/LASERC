...
LASERC<-function(dat,batch, mod=NULL,L=3,standardize=TRUE,center=NULL,scale=NULL, MaxIteration=200,lambda = log(n)/n, tau=0.3 * sqrt(1/(0.5*V)),
  ...
  Theta_new = update_func(dat,A,U,batch,XFmat,X,XGrp,sigma2,phi2,HGrp,lambda_ch = 0,p,V,L,order.temp.batch, tau=NULL,datGrp,n)
  ...
  while(Iter <= MaxIteration){
    ...
    Theta_new = update_func(dat,A,U,batch,XFmat,X,XGrp,sigma2,phi2,HGrp,lambda_ch = lambda,p,V,L,order.temp.batch, tau=tau,datGrp,n)
