...
update_func <- function(dat,A,U,batch,XFmat,X,XGrp,sigma2,phi2,Sig_AconY,HGrp,lambda_ch = 0.5,p,V,L,order.temp.batch,tau=0.3 * sqrt(log(L*V)/n),datGrp,n)
{
  ...
  b2<-t(b1)
  b3<-matrix(b2,ncol=L)
  b11<-sapply(1:nrow(A),function(i) (Umat^2)%*%diag(diag(Sig_AconY[[batch[i]]]),ncol=length(diag(Sig_AconY[[batch[i]]]))))
  b21<-t(b11)
